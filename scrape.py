...
# 取得した駅名からその駅の到着時刻を取得する(岡山から下関間)
def reply_next_time(station_name):
    # 現在時刻の取得
    now_time = {
    ...
    return {
        "up": get_next_time(now_time, html, "weekday-0"),
        "down": get_next_time(now_time, html, "weekday-1")
    }


def get_next_time(now_time, html, up_or_down):
...
# 終電 ===================================================
def reply_last_time(station_name):
    # 駅名のチェック
    url = get_station_url(station_name)
    if url == False:
        return False
    ...
    html = BeautifulSoup(res.content, "html.parser")
    return {
        "up": get_last_time(html, "weekday-0"),
        "down": get_last_time(html, "weekday-1")
    }


def get_last_time(html, up_or_down):
